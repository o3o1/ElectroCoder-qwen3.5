...
void print_trend(PID_Record records[], int count) {
    printf("=== PID Parameter History ===\n");
    printf("Timestamp(ms) | Kp     | Ki     | Kd     | Error  | Output\n");
    printf("--------------------------------------------------------\n");
    
    for (int i = 0; i < count; i++) {
        printf("%12lu | %6.2f | %6.2f | %6.2f | %7.2f | %6.2f\n",
               records[i].timestamp,
               records[i].Kp,
               records[i].Ki,
               records[i].Kd,
    ...
    if (count >= 3) {
        ...
        printf("\nMoving Average Output: %.2f\n", moving_avg);
    } else if (count > 0) {
        float sum = 0;
        for (int i = 0; i < count; i++) sum += records[i].output;
        printf("\nMoving Average Output: %.2f\n", sum / count);
    }
}
...
void analyze_overshoot(PID_Record records[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (target_error == 0) continue;
        
        float overshoot_percentage = (records[i].output - target_error) / target_error;
        
        if (overshoot_percentage > overshoot_threshold) {
            if (overshoot_count == 0) {
                printf("Overshoot Records Found:\n");
                printf("Timestamp(ms) | Output | Target Error | Overshoot %%\n");
                printf("--------------------------------------------\n");
            }
            printf("%12lu | %6.2f | %12.2f | %10.1f%%\n",
                   records[i].timestamp,
                   records[i].output,
                   target_error,
