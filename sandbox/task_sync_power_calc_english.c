...
typedef struct {
    ...
    float u_alpha_prev; // Previous α-axis voltage
    float u_beta_prev;  // Previous β-axis voltage
    float integral;     // Integrator state
    float kp;           // Proportional coefficient
    float ki;           // Integral coefficient
} PLLessSync;

// Clarke transform: three-phase abc to αβ coordinate system
...
void plless_sync_update(PLLessSync *sync, float ua, float ub, float uc) {
    ...
    // 2. Calculate instantaneous reactive power q = uα * uβ' - uβ * uα'
    ...
    // 8. Normalize phase angle to [-π, π] range
